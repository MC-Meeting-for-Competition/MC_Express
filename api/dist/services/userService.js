...
export const login = async (loginDto) => {
    ...
    const token = jwt.sign({ id: user.id, email: user.email }, "jwt_secret_key_abcdefghijklmnopqrstuvwxyz", // 비밀 키는 환경 변수로 관리하는 것이 좋습니다.
    { expiresIn: "1d" });
    return token;
};
export const verifyToken = async (token) => {
    const decoded = jwt.verify(token, "jwt_secret_key_abcdefghijklmnopqrstuvwxyz");
